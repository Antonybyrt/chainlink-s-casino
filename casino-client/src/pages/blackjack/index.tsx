...
function BlackjackComponent() {
  ...
  const [balance, setBalance] = useState(balanceData ? Number(balanceData.formatted) : 0);
  ...
  const canDeal   = !gameStarted && isRegistered;
  const canDouble = hookCurrentBet !== null && casinoBalance >= hookCurrentBet * 2 && playerCards.length === 2;
  const canSplit  = playerCards.length === 2 && (playerCards[0] % 13) === (playerCards[1] % 13);
